...
export default function CrearComponenteRed({
  onSuccess,
  onClose,
}: {
  onSuccess: () => void;
  ...
  return (
    ...
    >
      <header className="p-6 grid gap-4">
        <h4 className="text-[28px]">Crear componente de red</h4>
        <p>
      ...
      >
        ...
        <Select
          ...
          label="Red"
          onChangeValue={(value) =>
            setRed(redes.find((red) => red.id === +value)!)
          }
          options={redes
        ...
        />
        <Select
          disabled={isLoadingFuentes || !tipoComponente || !red}
          name={"refSourceId" as FormItem}
          label="Fuente"
          options={fuentes
            .filter((fuente) => fuente.status === 1)
            .filter(
              (fuente) => fuente.refComponentTypeId === tipoComponente?.id,
            )
            .filter((fuente) => fuente.refNetworkId === red?.id)
            .map((fuente) => ({
              text: fuente.label,
