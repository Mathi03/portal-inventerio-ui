                          ...
                          <Fragment key={`nested-${parentAttrKey}`}>
                            <div style={{ gridColumn: '1 / -1', marginBottom: '10px' }}>
                              <h5 className={`text-[15px] font-semibold ${className}`}>
                                {parentAttributeConfig.label || parentAttributeConfig.name}
                              </h5>
                            </div>
                            {parentAttributeConfig.atribs_config?.map((nestedAttributeConfig, nestedAttrKey) => (
                              <div key={nestedAttrKey}>
                                <InputDynamic
                                  ...
                                  value={
                                    (attribute &&
                                      typeof attribute[parentAttributeConfig.name] === 'object' &&
                                      attribute[parentAttributeConfig.name] !== null &&
                                      (attribute[parentAttributeConfig.name] as NestedAttributes)[nestedAttributeConfig.name])
                                      ? (attribute[parentAttributeConfig.name] as NestedAttributes)[nestedAttributeConfig.name]
                                      : ''
                                  }
                                  onChange={(name, value) =>
            ...
            <div>
              <div>
                ...
                {tipoComponente.configData.map((configData, configDataKey) => {
                  ...
                  return (
                    <div key={configDataKey+"guille"} style={{ marginBottom: '15px' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px' }}>
                        {/* Renderizar atributos regulares primero */}
                        ...
                        {/* Renderizar atributos con atribs_config al final */}
                        {/* Renderizar atributos con atribs_config */}
                        {nestedConfigServices.map((parentServiceConfig, parentAttrKey) => (
                          <Fragment key={`nested-${parentAttrKey}`}>
                            <div style={{ gridColumn: '1 / -1', marginBottom: '10px' }}>
                              <h5 className={`text-[15px] font-semibold ${className}`}>
                            ...
                            {parentServiceConfig.atribs_config?.map((nestedServiceConfig, nestedAttrKey) => (
                              <div key={nestedAttrKey+"guille"}>
                                <InputDynamic
                                  {...nestedServiceConfig}
                                  value={
                                    (service &&
                                      typeof service[parentServiceConfig.name] === 'object' &&
                                      service[parentServiceConfig.name] !== null &&
                                      (service[parentServiceConfig.name] as NestedServices)[nestedServiceConfig.name])
                                      ? (service[parentServiceConfig.name] as NestedServices)[nestedServiceConfig.name]
                                      : ''
                                  }
                                  onChange={(name, value) =>
