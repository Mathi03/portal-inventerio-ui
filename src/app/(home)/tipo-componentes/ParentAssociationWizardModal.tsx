...
function ParentAssociationWizardModal({
  ...
  redesPadre: any[];
  redesHijo: any[];
  onSave: (association: { parentId: number; parentRedId: number; childRedId: number, childType: string }) => void;
  initialStep?: number;
  initialParent?: any;
  ...
  const [selectedParentRed, setSelectedParentRed] = useState<any>(initialParentRed);
  const [selectedChildRed, setSelectedChildRed] = useState<any>(initialChildRed);
  const { getTipoComponentes, tipoComponentes, allTipoComponente, allTipoComponentes} = useTipoComponente();
  ...
  const [networksId, setNetworksId] = useState<any[]>([])
    ...
    useEffect(() => {
    getTipoComponentes({ search:searchParent, page, limit:1000 });
    }, [searchParent]);
...
if (step === 1) {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
        ...
        />
          <div className="flex-1 overflow-auto border rounded-md p-2" style={{maxHeight: 200}}>
            <ul>
              { tipoComponentes.filter(tc => tc.id  != tipoComponenteId).map(tc => (
                <li key={tc.id} className="border border-gray-200 rounded hover:bg-gray-50 transition-colors">
                  <button
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      ...
      <div className="flex-1 overflow-auto border rounded-md p-2" style={{maxHeight: 200}}>
        <ul>
          {redesPadre.filter( red => networksId?.includes(red.id)).length>0 ? (redesPadre.filter( red => networksId?.includes(red.id))?.map(red => 
            <li key={red.id} className="border border-gray-200 rounded hover:bg-gray-50 transition-colors">
              <button
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
        <div className="bg-white rounded-xl shadow-2xl p-8 min-w-[400px] w-[800px] min-h-[220px] flex flex-col gap-6 relative">
          ...
          <div className="border border-gray-300 rounded-lg p-4 mb-4">
            <h4 className="font-semibold text-[#0057b8] mb-2">Red del tipo de componente hijo</h4>
            <ul>
              {redesHijo.filter(redHija =>
                !parentAssociations.some(association =>
                  association.parentRedId === selectedParentRed.id &&
                  association.childRedId === redHija.id
                )
              ).length > 0 ? (redesHijo.filter(redHija =>
                !parentAssociations.some(association =>
                  association.parentRedId === selectedParentRed.id &&
                  association.childRedId === redHija.id
                )
